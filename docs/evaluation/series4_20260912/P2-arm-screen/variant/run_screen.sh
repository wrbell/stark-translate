...
set -o pipefail
ROOT=/Users/willem/Code/vibes/SRTranslate; P2=$ROOT/.cache/series4-20260912/P2/variant; X=$ROOT/.cache/followup-20260911/X
DIR=${P2_CHECKOUT:-/Users/willem/Code/vibes/SRTranslate-wt-p2b}; PY=$ROOT/venv/bin/python
...
run_one() {
  ...
  tag=p2c0912_${clip}_${arm}_r${rep}
  ...
  case $arm in
    ctl) (cd $DIR && env -u STARK_EXPERIMENT_PARTIAL_REUSE_MS -u STARK_EXPERIMENT_PARTIAL_REUSE_KEEP_CONFIDENCE $PY -m tools.replay_bench --manifest $X/manifest_${clip}.json --tag $tag --configs "$arm=$EXTRA" 2>&1 | tail -3) ;;
    reuse100c) (cd $DIR && STARK_EXPERIMENT_PARTIAL_REUSE_KEEP_CONFIDENCE=true STARK_EXPERIMENT_PARTIAL_REUSE_MS=100 $PY -m tools.replay_bench --manifest $X/manifest_${clip}.json --tag $tag --configs "$arm=$EXTRA" 2>&1 | tail -3) ;;
    reuse300c) (cd $DIR && STARK_EXPERIMENT_PARTIAL_REUSE_KEEP_CONFIDENCE=true STARK_EXPERIMENT_PARTIAL_REUSE_MS=300 $PY -m tools.replay_bench --manifest $X/manifest_${clip}.json --tag $tag --configs "$arm=$EXTRA" 2>&1 | tail -3) ;;
  esac
...
order=0
for clip in A B; do
  for run in ctl:0 reuse300c:0 reuse300c:1 ctl:1 ctl:2 reuse300c:2; do
    run_one $clip ${run%%:*} ${run##*:} $order; order=$((order+1))
  done
done
echo "=== P2c screen done $(date -u +%FT%TZ) ==="
