"""RUNS.json for tools/tail_screen_report.py from the P2 screen's runs.jsonl (single checkout, metrics under it)."""
import glob, json, pathlib, sys
ROOT = pathlib.Path('/Users/willem/Code/vibes/SRTranslate'); P2 = ROOT / '.cache/series4-20260912/P2'
proto = json.loads((P2 / 'protocol.json').read_text())
runs = [json.loads(l) for l in (P2 / 'runs.jsonl').read_text().splitlines() if l.strip()]
...
out = {'screen': proto['id'], 'clips': {k: {'path': str(ROOT / v['path']), 'sha256': v['sha256']} for k, v in proto['clips'].items()},
       'arms': {name: ({'kind': 'control'} if a['kind'] == 'control' else {'kind': 'candidate', 'memory_rule': 'relative'}) for name, a in proto['arms'].items()}, 'runs': []}
for r in runs:
    if r['rc'] != 0: continue
...
(P2 / 'RUNS.json').write_text(json.dumps(out, indent=2) + '\n'); print('runs', len(out['runs']))
