...
|---|---|---|---|---|---|---|
| S0 pre-flight (caffeinate, worktrees, status branch) | RUNNING | 2026-09-11T19:05Z | | | | |
| P2-H harness PR (Codex): cache-limit consistency, `partial_recheck_translation`, cpu_ms + first_stream_token trace, attribution tool, aligned G4, clip prep | PENDING | | | | | |
| P2-L launcher PR (Codex): launchd honours `.stark-python`, bootstrap protects `stt_env`, `check_dependency_audit --runtime mac`, `scripts/audit_mac_runtime.sh` | PENDING | | | | | |
| P3-C coverage gate 50 → 65 | PENDING | | | | | measured 68.17 % on main |
| P3-S installed smoke + Mac runtime audit | PENDING | | | | | |
| P1-H hymn #193 second control + slices | PENDING | | | | | first control ran 2026-09-10 on c13f51f |
