...
def main():
    ...
    parser.add_argument("--explicit-voice-paths", action="store_true", help="Retest explicit local snapshot symlinks")
    ...
    try:
        ...
        if args.play_existing:
            ...
            # Exercise the ordinary stock-name path after proving both voices
            # already exist locally; retain snapshot filenames beside configs.
            engine = PiperTTSEngine(voices=resolved if args.explicit_voice_paths else voices)
            engine.load()
        for language in languages:
            if args.play_existing:
