...
Validation now includes **1,722 CPU tests passed, 2 skipped**, 57.83% coverage, **3 cached MLX GPU tests passed**, Ruff/mypy, and zero HTML5 Tidy warnings across all five displays. CI-configured security checks pass; an expanded Bandit run still reports 26 medium B615 model-revision pinning findings. The full Mac runtime extras were installed and import-tested in an isolated environment, preserving `stt_env`; final v2026.14 artifacts await source freeze.
...
3. **Installation:** backend-aware setup/preflight, shared pinned cache resolution, default EN/ES Piper voices, generated launchd install/uninstall and wheel/source-bundle guards are implemented. A preliminary installed wheel starts outside the checkout; final wheel/sdist/Mac ZIP build and reinstall remain pending.
...
5. **Experiments — running:** 48 frozen runs cover eight configurations × two models × three alternating pairs on a 45-second English clip: baseline, idle warmups, final-aware partial scheduling, 0.4/0.35 s silence versus 0.5 s baseline, conservative Marian routing, terminology examples and ONNX VAD. Cadence stays 0.6 s. No setting is promoted before report review; longer and natural Spanish validation still apply. MTP research is deferred.
