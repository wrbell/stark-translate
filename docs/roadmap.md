...
The last full validation includes **1,722 CPU tests passed, 2 skipped**, 57.83% coverage, **3 cached MLX GPU tests passed**, Ruff/mypy, and zero HTML5 Tidy warnings across all five displays. These full-suite figures precede the latest VAD/CT2 setup changes, whose focused checks and real offline CPU loads/conversions passed. CI-configured security checks pass; the expanded Bandit run reports 26 medium B615 findings with [scope and remaining pinning limitations documented](evaluation/mac_v2026_14_security.md). The full Mac runtime extras were installed and import-tested in an isolated environment, preserving `stt_env`; final full-suite checks and v2026.14 artifacts remain pending.
