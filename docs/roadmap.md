...
Final CPU validation after the setup changes includes **1,785 passed, 4 skipped**, 59.05% coverage, Ruff/mypy, and zero HTML5 Tidy warnings across all five displays. The earlier **3 cached MLX GPU tests passed**; later 24 synthetic routing replays separately cover post-setup pipeline execution. Real offline VAD CPU loads and isolated CT2 conversion/reuse also passed. CI-configured security checks pass; the final expanded Bandit run reports 27 medium B615 findings: the original 26 call sites plus one guarded full-commit download, with [scope and remaining pinning limitations documented](evaluation/mac_v2026_14_security.md). The full Mac runtime extras were installed and import-tested in an isolated environment, preserving `stt_env`; final v2026.14 artifacts remain pending.
