...
The [API rehearsal script](../scripts/dry_run_rehearsal.sh) starts real sessions using the operator's configured input source. Run it with exclusive use of an idle operator. It waits up to 120 seconds per startup/restart/stop (set `REHEARSAL_TIMEOUT_S=180` for a longer cold start), checks fresh input readiness and distinct EN→ES→EN session identities, and requires a completed final stop. Failure or interruption triggers cleanup only when the current session still matches its own start response. A changed or unknown session identity is left untouched; inspect the operator before retrying. This is a control/API check, not a caption-quality, hymn, audience-rendering or physical-device acceptance test.
...
Pause and Resume use a session-scoped command file with an acknowledged sequence. Pause closes the live capture child. Resume opens a fresh microphone child and retains the original capture sample coordinate system. File replay freezes while paused and resumes from the last consumed original sample; prefetched frames are replayed rather than skipped, and the file does not restart from the beginning. Stop requests cooperative shutdown before escalating to process termination; the runner reaps owned children, including capture and runtime-server descendants.
