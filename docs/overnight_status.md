...
## 2026-09-12 series 3 (series3_20260912)

Recorded in [`docs/evaluation/series3_20260912/STATUS.md`](evaluation/series3_20260912/STATUS.md): latency attribution on the promoted runtime (a third of Gemma decode time overlaps preview STT; first translated tokens reach the wire at ~1.1–1.2 s), the declared L-B screen (all three admitted arms REJECTED on G1 on both clips — marian_threads_2 raised the Gemma silence p95 (+16 % clip A, +6 % clip B); max_utterance_6 lowered medians (-11 %/-6 %) but raised the clip A tail (+28 %) and failed its quality guard (20 % concatenated Spanish WER, duplicated finals); partial_recheck_translation suppressed the measured overlap (78-82 partials per run on A, 31-34 on B) and cut the clip A Gemma p95 by 8.8 % (below the 15 %/300 ms gate) with nothing on B and previews -7…-13 %. Screen without p95 claim (45 Gemma-routed silence finals on clip A).), the hymn #193 second control and search slices, a clean 3,640 s service replay on the promoted runtime, the coverage gate raised to 65 %, launcher pointer support in launchd/bootstrap, and a runnable Mac runtime audit. The closed-arm registry lives in [`docs/latency_next_experiments.md`](latency_next_experiments.md). Defaults are unchanged.
