...
- **Confidence flagging:** Whisper finals retry with the fallback model when `avg_logprob < -1.2` or `compression_ratio > 2.4`; words with probability `< 0.5` are listed as low-confidence; fallback events go to the active-learning JSONL. Parakeet confidence is a TDT proxy.
- **Music hold:** `--music-threshold` / `--music-holdoff` mute STT during hymns; displays receive `music_hold`.
