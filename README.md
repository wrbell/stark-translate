...
**Next (Sept 2026, v2026.13 — Mac latency program):** fix the Gemma 4 stop-token bug that made every Mac final run to `max_tokens` (#172), add a real-audio replay benchmark (#179), Gemma 4 MTP drafter on MLX (#177), Parakeet TDT v3 STT (#178, WER-gated), Marian CTranslate2 on Mac; target < 1.0 s true end-to-end finals from ~3.2 s. Then a CUDA latency proposal for the A2000 box (Gemma 4 MTP in llama.cpp ≥ b10883 with f16 KV; #173, #174), then the deferred 9.4.1 TTS routing (#132) and 9.6.1 live diarization (#133). WSL-side Phase 4 → E4B SFT → W17 → Mac transfer stays blocked on the A2000 box (`docs/wsl_pipeline_refresh.md`). Phase 1C EAGLE-3 remains deferred (single-GPU spec decode showed no benefit).
