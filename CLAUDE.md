...
- **Mac (MLX):** mlx-whisper STT + Gemma 4 OptiQ E4B finals (TranslateGemma opt-out via `--model-family translategemma`); MarianMT-HF on PyTorch CPU. The ~2.2 s Mac finals measured 2026-08-30 were inflated by the Gemma 4 EOS bug (#172); v2026.13 targets ≤ 0.6 s finals / < 1.0 s true end-to-end.
...
- **Replay benchmark** (#182): `dry_run_ab.py --audio-file` + `tools/replay_bench.py` play real sermon clips through VAD → partials → finals and report p50/p95 (`e2e_latency_ms` = speech end → final). Pick VAD-dense clip offsets (the 10-min mark is a hymn → music hold). Results: `docs/archive/v2026.13/MAC_LATENCY.md`.
...
- **Measured (speech-end → final p50, two 150 s clips):** 2026-08-30 code 4.8 / 3.7 s → fixes 4.0 / 2.9 s → + Parakeet 1.6 / 1.0 s; partials ~1.0 s → ~0.2–0.26 s. Fast mode `--gemma4-size e2b`: 1.2 / 0.86 s (canary 11/18 vs 13/18). Isolated E4B translate: 2139/2176/3242 → 473/1393/1677 ms. VAD 0.35 s / 0.4 s cadence tried and rejected (more GPU contention).
