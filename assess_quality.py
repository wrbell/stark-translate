...
def sample_for_assessment(transcript_dir, n=100, output_path="assessment_sample.jsonl",
                          seed=42):
    ...
    print(f"\nNext steps:")
    print(f"  1. Listen to each segment's audio")
    print(f"  2. Fill in the 'manual_text' field with your transcription")
...
def compute_baseline_wer(assessment_path):
    ...
    if overall_wer < 0.10:
        print(f"  < 10% WER: Use directly with confidence-based filtering")
    elif overall_wer < 0.20:
        print(f"  10-20% WER: Filter worst segments by avg_logprob")
    elif overall_wer < 0.30:
        print(f"  20-30% WER: Weakly supervised pretrain + fine-tune on clean subset")
    else:
        print(f"  > 30% WER: Re-transcribe everything with Whisper large-v3")
...
def main():
    parser = argparse.ArgumentParser(
        description="Baseline transcript quality assessment"
    )
    subparsers = parser.add_subparsers(dest="command")

    # Sample command
    sample_parser = subparsers.add_parser("sample",
                                           help="Generate sample for manual review")
    sample_parser.add_argument("--input", "-i", default="stark_data/transcripts",
                                help="Directory with JSON transcripts")
    sample_parser.add_argument("--n", type=int, default=100,
                                help="Number of segments to sample")
    sample_parser.add_argument("--output", "-o", default="assessment_sample.jsonl",
                                help="Output JSONL path")
    sample_parser.add_argument("--seed", type=int, default=42)

    # Evaluate command
    eval_parser = subparsers.add_parser("evaluate",
                                         help="Compute WER after manual transcription")
    eval_parser.add_argument("--input", "-i", default="assessment_sample.jsonl",
                              help="JSONL with manual_text filled in")
