...
def stub_pipeline(tmp_path: Path) -> Path:
    ...
    stub.write_text(
        ...
        "    session_id = datetime.now().strftime('%Y%m%d_%H%M%S') + '_' + args.lang\n"
        "    csv_path = os.path.join('metrics', f'ab_metrics_{session_id}.csv')\n"
        "    # Match dry_run_ab's 'pid file' approach: write the session_id into\n"
        "    # a deterministic location so the test can find the CSV the runner\n"
        "    # actually computes (which uses datetime.now too, so they'll diverge).\n"
        "    with open(csv_path, 'w', newline='') as f:\n"
        "        w = csv.writer(f)\n"
        "        w.writerow(['chunk_id','english','spanish','stt_ms','translate_ms','latency_ms','confidence'])\n"
        "        for i in range(3):\n"
        "            w.writerow([i, f'sample {i}', f'muestra {i}', 100.0, 200.0, 300.0, 0.9])\n"
        "            f.flush()\n"
        "            time.sleep(0.05)\n"
